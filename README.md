...
Alternative Considered: Embeddings API
Rejected: 100x more expensive, requires internet, vendor lock-in

#### 2. Pre-computed Document Embeddings
Decision: Embed all documents offline, only embed queries in real-time
Rationale:

37k documents × 1024 dimensions = 150MB storage (manageable)
...
Used for: Raspberry Pi deployments where size matters

#### 5. RRF Over Weighted Averaging
Decision: Use Reciprocal Rank Fusion instead of score-based weighting
Rationale:

Score Normalization: BM25 (0-15) and cosine similarity (0-1) have different scales
Rank Stability: Rankings more stable than raw scores across different queries
Parameter-Free: RRF requires no tuning vs. weight optimization
Empirical Results: 15-20% better relevance vs. weighted averaging
